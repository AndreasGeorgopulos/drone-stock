...
class RoleController extends Controller
{
	...
	public function edit (Request $request, $id = 0) {
		...
		if ($request->isMethod('post')) {
			...
			if ($validator->fails()) {
				return redirect(route('admin_roles_edit', ['id' => $id]))->withErrors($validator)->withInput()->with('form_warning_message', [
					trans('admin.Sikertelen mentés'),
					trans('admin.A jogosultság adatainak rögzítése nem sikerült a következő hibák miatt:')
				]);
			}
			...
			return redirect(route('admin_roles_edit', ['id' => $model->id]))->with('form_success_message', [
		...
		if ($model->id) {
			...
			foreach ($routeCollection as $value) {
				if (Str::contains($value->getName(), 'admin')) {
	...
	public function delete ($id) {
		if ($model = Role::find($id)) {
			...
			$model->users()->delete();
			$model->delete();
			return redirect(route('admin_roles_list'))->with('form_success_message', [
				trans('admin.Sikeres törlés'),
				trans('admin.A jogosultság sikeresen el lett távolítva.')
			]);
		}
	}
}
