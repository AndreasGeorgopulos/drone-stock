...
@section('content_header')
    <h1>{{trans('admin.Fordítások')}}</h1>
@stop

@section('content')
    <div class="panel panel-transparent">
        <p>
        {{trans('admin.Figyelem, a fordítások nem láthatóak addig, amíg nincsenek exportálva az app/lang-ba a "php artisan translation:export" paranccsal vagy a Közzététel gombbal.')}}</p>
        <div class="alert alert-success success-import" style="display:none;">
            <p>{{trans('admin.Sikeres importálás, feldolgozva')}} <strong class="counter">N</strong> {{trans('admin.elem! Frissítsd az oldalt, hogy láthasdd a változásokat!')}}</p>
        </div>
        <div class="alert alert-success success-find" style="display:none;">
            <p>{{trans('admin.Fordítások keresése sikeres, találtunk')}} <strong class="counter">N</strong> {{trans('admin.element!')}}</p>
        </div>
        <div class="alert alert-success success-publish" style="display:none;">
            <p>{{trans('admin.Fordítások közzététele sikeres a megadott csoporthoz:')}} '<?= $group ?>'!</p>
        </div>
        ...
        <p>
        <?php if(!isset($group)) : ?>
        <form class="form-inline form-import" method="POST" action="<?= action('\App\Http\Controllers\Admin\TranslationController@postImport') ?>" data-remote="true" role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <select name="replace" class="form-control">
                <option value="0">{{trans('admin.Hozzáfűzzük az új fordításokat')}}</option>
                <option value="1">{{trans('admin.Meglévő fordítások cseréje')}}</option>
            </select>
            <button type="submit" class="btn btn-success"  data-disable-with="{{trans('admin.Betöltés')}}..">{{trans('admin.Importálás')}}</button>
        </form>
        <form class="form-inline form-find" method="POST" action="<?= action('\App\Http\Controllers\Admin\TranslationController@postFind') ?>" data-remote="true" role="form" data-confirm="{{trans('admin.Biztosan elindítod a keresést? Minden fordítás bekerül az adatbázisba.')}}">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <p></p>
            <button type="submit" class="btn btn-info" data-disable-with="{{trans('admin.Keresés')}}.." >{{trans('admin.Fordítások keresése a fájlokban')}}</button>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
        <form class="form-inline form-publish" method="POST" action="<?= action('\App\Http\Controllers\Admin\TranslationController@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="{{trans('admin.Biztosan közzéteszi a(z) ')}} '<?= $group ?>? {{trans('admin.Ezzel felülírja a jelenlegi fordításokat!')}}">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >{{trans('admin.Közzététel')}}</button>
            <a href="<?= action('\App\Http\Controllers\Admin\TranslationController@getIndex') ?>" class="btn btn-default">{{trans('admin.Vissza')}}</a>
        </form>
        <?php endif; ?>
        </p>
        <form role="form">
        ...
        <?php if($group): ?>
        <form action="<?= action('\App\Http\Controllers\Admin\TranslationController@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <textarea class="form-control" rows="3" name="keys" placeholder="{{trans('admin.Kulcs hozzáadása')}}"></textarea>
            <p></p>
            <input type="submit" value="Add keys" class="btn btn-primary">
        </form>
        <hr>
        <h4>{{trans('admin.Összesen')}}: <?= $numTranslations ?>, {{trans('admin.változott')}}: <?= $numChanged ?></h4>
        <table class="table">
            <thead>
            <tr>
                <th width="15%">{{trans('admin.Kulcs')}}</th>
            ...
            <tr id="<?= $key ?>">
                ...
                <?php if($deleteEnabled): ?>
                <td>
                    <a href="<?= action('\App\Http\Controllers\Admin\TranslationController@postDelete', [$group, $key]) ?>" class="delete-key" data-confirm="Are you sure you want to delete the translations for '<?= $key ?>?"><span class="glyphicon glyphicon-trash"></span></a>
                </td>
                <?php endif; ?>
            </tr>
            <?php endforeach; ?>
        ...
        <?php else: ?>
        <p>{{trans('admin.Válassz egy csoportot a fordításhoz. Ha nem látható a csoport, győződj meg róla, hogy importáltad a fordításokat.')}}
   </p>
    ...
    <script>
        $(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?= action('\App\Http\Controllers\Admin\TranslationController@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?= action('\App\Http\Controllers\Admin\TranslationController@getIndex') ?>';
                }
            });
