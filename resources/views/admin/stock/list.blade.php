            ...
            <div class="col-sm-12">
                <table class="table table-bordered table-striped dataTable">
                    ...
                    <tr role="row">
                        ...
                        <th class="hidden-xs" data-column="category_id">{{trans('admin.Kategória')}}</th>
                        <th class="hidden-xs text-center">{{trans('admin.File-ok')}}</th>
                    ...
                    @foreach ($list as $model)
                        ...
                        <tr role="row" class="odd">
                            ...
                            <td>{{$model->id}}</td>
                            <td>{{$model->name}}</td>
                            <td class="hidden-xs">{{$model->category ? $model->category->name : ''}}</td>
                            <td class="hidden-xs text-center">{{$model->sizes()->count()}}</td>
                                ...
                                <div class="btn-group pull-right">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        ...
                                        @foreach(config('app.languages') as $lang)
                                            <li><a href="{{url('/video/' . $model->category->translates()->where('language_code', $lang)->first()->slug)}}/{{$model->translates()->where('language_code', $lang)->first()->slug}}" target="_blank"><i class="fa fa-external-link"></i> {{str_replace('[LANG]', trans('admin.' . $lang), trans('admin.Ugrás a [LANG] oldalra'))}}</a></li>
                                        @endforeach
