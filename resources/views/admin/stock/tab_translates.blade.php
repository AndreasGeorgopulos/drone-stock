@foreach (config('app.languages') as $lang)
    @php
        ...
        if (empty($translate)) {
            $translate = new \App\Stock_Translate();
            $translate->active = 1;
        }
    @endphp
    <div class="tab-pane @if(old('tab', 'general_data') == $lang . '_data') active @endif" id="{{$lang}}_data">
        ...
        <div class="form-group col-md-12">
            ...
            <textarea name="translate[{{$lang}}][lead]" class="form-control wysig">{{old('translate.' . $lang . '.lead', !empty($translate->lead) ? $translate->lead : '')}}</textarea>
        </div>

        <div class="form-group col-md-12">
            <label>{{trans('admin.Szövegtörzs')}}</label>
            <textarea name="translate[{{$lang}}][body]" class="form-control wysig">{{old('translate.' . $lang . '.body', !empty($translate->body) ? $translate->body : '')}}</textarea>
        </div>
